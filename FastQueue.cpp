...
int main(int argc, char* argv[])
{
    ...
    std::unique_ptr<TransactionLockManager> tlm = std::unique_ptr<TransactionLockManager>(new TransactionLockManager());
    std::unique_ptr<TransactionHandler> th = std::unique_ptr<TransactionHandler>(
        new TransactionHandler(qm.get(), tlm.get(), cm.get(), fh.get(), cache_handler.get(), pm.get(), controller.get()->get_cluster_metadata(), response_mapper.get(), transformer.get(), util.get(), settings.get(), server_logger.get())
    );
