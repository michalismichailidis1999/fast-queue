...
private:
	...
	void set_partition_active_segment(Partition* partition, const std::string& queue_name, bool is_cluster_metadata_queue);

	void set_segment_index(const std::string& queue_name, PartitionSegment* segment, int partition = -1);

public:
	BeforeServerStartupHandler(QueueManager* qm, ClusterMetadata* cluster_metadata, ClusterMetadata* future_cluster_metadata, FileHandler* fh, QueueSegmentFilePathMapper* pm, Util* util, Logger* logger, Settings* settings);
