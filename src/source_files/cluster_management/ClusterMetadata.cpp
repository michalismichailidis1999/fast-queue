...
void ClusterMetadata::apply_command(Command* command) {
	...
	case CommandType::CREATE_QUEUE: {
		...
		std::shared_ptr<QueueMetadata> queue_metadata = std::shared_ptr<QueueMetadata>(
			new QueueMetadata(
				command_info->get_queue_name(),
				command_info->get_partitions(),
				command_info->get_replication_factor()
			)
		);
	...
	case CommandType::ALTER_PARTITION_ASSIGNMENT: {
		...
		int node_id = command_info->get_to_node();

		auto node_queues = this->nodes_partitions[node_id];

		auto owned_partitions = this->owned_partitions[command_info->get_queue_name()];
		...
		if (owned_partitions == nullptr) {
			...
			this->owned_partitions[command_info->get_queue_name()] = owned_partitions;
		}

		auto partition_owners = (*(owned_partitions.get()))[command_info->get_partition()];

		if (partition_owners == nullptr) {
			partition_owners = std::make_shared<std::unordered_set<int>>();
			(*(owned_partitions.get()))[command_info->get_partition()] = partition_owners;
		}

		auto node_partitions = (*(node_queues.get()))[command_info->get_queue_name()];

		if (node_partitions == nullptr) {
			node_partitions = std::make_shared<std::unordered_set<int>>();
			(*(node_queues.get()))[command_info->get_queue_name()] = node_partitions;
		}

		node_partitions->insert(command_info->get_partition());
		...
		if (command_info->get_from_node() <= 0) return;

		node_id = command_info->get_from_node();

		node_queues = this->nodes_partitions[node_id];

		node_partitions = (*(node_queues.get()))[command_info->get_queue_name()];

		node_partitions->erase(command_info->get_partition());

		if (node_partitions->size() == 0)
			node_queues.get()->erase(command_info->get_queue_name());
	...
	case CommandType::ALTER_PARTITION_LEADER_ASSIGNMENT: {
		...
		auto partitions_leaders = this->partition_leader_nodes[command_info->get_queue_name()];

		if (partitions_leaders == nullptr) return;

		int node_id = command_info->get_new_leader();

		int lead_partitions_count = this->nodes_leader_partition_counts->get(node_id);

		(*(partitions_leaders.get()))[command_info->get_partition()] = node_id;

		this->nodes_leader_partition_counts->update(node_id, lead_partitions_count + 1);

		if (command_info->get_prev_leader() <= 0) return;

		node_id = command_info->get_prev_leader();
